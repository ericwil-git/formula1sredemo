...
Status: in development — Bicep + app code being scaffolded with VSCode + GitHub Copilot. Owner: Eric Wilson — Principal Cloud Solution Architect, Microsoft Region: Central US
...
The data is real Formula 1 timing and telemetry from the open-source FastF1 library — so the demo is grounded in something a sports-media audience will recognize instantly.
...
+------------------+                      +-----------+-------------+
                                  ...
                                  + Observability Agent
Full architecture, sequence diagrams, and component contracts live in docs/techspec.md.
...
Detailed deployment, parameter shapes, and post-deploy steps are in docs/techspec.md §8.
...
Demo flow
Open the architecture diagram — explain the three tiers.
RDP to the VM, show the FileGenerator console and the Scheduled Task for ingestion.
Trigger an ingestion for a single race and watch Prometheus counters climb in Grafana.
Open the web app, navigate to Race → 2024 → Monaco — show the network call hitting the VM and the cached CSV on D:\f1-files\.
Open the Lap Explorer — overlay Verstappen vs Leclerc telemetry.
The SRE moment — stop the FileGenerator service, reload the page, and ask the Observability Agent "what's wrong with the F1 demo?" — it pulls from Log Analytics and Monitor Workspace and points at the failed service.
Full storyline in docs/techspec.md §9.
...
Disclaimer
This is a Microsoft-internal customer demo. Formula 1 data is sourced via the FastF1 library (MIT license). F1, Formula 1, and team / driver names are trademarks of their respective owners and are used here for non-commercial demonstration purposes only.

Contact
Eric Wilson — eric.wilson@microsoft.com Principal Cloud Solution Architect, Microsoft
